...
mejor_modelo2 = ModelPredic(nombre="", prediccion=0)
    
@app.post("/procesar_datos/")
async def procesar_datos(data: InputData):
    global carrera, semestre, mejor_modelo2
    carrera = data.carrera
    semestre = data.semestre
    variables = getVariables(carrera, int(semestre))
    modelo = cargar_entrenar_modelo(carrera, semestre)
    mejor_modelo2.nombre = modelo
    return {"variables": variables, "modelo": modelo}
...
async def predict(data: dict):
    ...
    if mejor_modelo:
        try:
            ...
            mejor_modelo2.prediccion = prediction
            return [mejor_modelo2]
...
def cargar_entrenar_modelo(carrera, semestre):
    global mejor_modelo, variables_por_carrera
    try:
        ...
        columnas_filtradas = []
        for e in variables:
            columnas_filtradas.append(e['codigo'])
...
def getVariables(carrera: str, semestre: int):
    ...
    def cargarDiccionario():
        file = f'DiccionarioDatosUD.xlsx'
        ruta_archivo = os.path.join(cwd, file)
        datos = pd.read_excel(ruta_archivo)
        return datos
    
    fileDic = cargarDiccionario()
    
    C = fileDic
    C1 = C.copy(deep=True)
    
    C_values1=pd.concat([C1],axis=0)
    C_values1.fillna(0,inplace=True)
    
    diccionario = C_values1.to_dict(orient='records')
    # print("***************************************")
    # print("diccionario")
    # print(diccionario)
    # print("***************************************")
    ...
    X_values1=pd.concat([X1],axis=0)
    X_values1.fillna(0,inplace=True)
    
    variables = []
    # variables_por_carrera = X_values1.columns.tolist()
    
    # print(variables_por_carrera)
    
    semestreString = numero_a_letras(semestre)
    
    # for columna in variables_por_carrera:
    #     for item in diccionario:
    #         if columna == item['CODIGO'] and item['CODIGO']:
    #             variables.append({"codigo": item['CODIGO'], "descripcion": item['DESCRIPCION']})
                
    # Version con array quemado
    for columna in variables_por_carrera[carrera][str(semestre)]:
        for item in diccionario:
            if columna == item['CODIGO']:
                variables.append({"codigo": item['CODIGO'], "descripcion": item['DESCRIPCION']})
