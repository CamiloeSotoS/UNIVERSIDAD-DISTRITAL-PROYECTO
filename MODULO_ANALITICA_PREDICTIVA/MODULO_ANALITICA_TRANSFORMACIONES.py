...
router = APIRouter(prefix='/predictiva/transformaciones')
...
async def procesar_datos(data: InputData):
    ...
    return {"data": imagenes_base64, "csv": diccionario_dataframes[0]}


@router.get("/getVariables/{carrera}/{semestre}")
async def getVariables(carrera: str, semestre: int):
    ...
    for columna in X_values1.columns.tolist():
